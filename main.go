...
func main() {
	var (
		...
		password = envflag.String("MYSQL_PASSWORD", "root", "address where mysql db is listening")
		host     = envflag.String("MYSQL_HOST", "mysql2", "address where mysql db is listening")
		port     = envflag.String("MYSQL_PORT", "3306", "address where mysql db is listening")
		database = envflag.String("MYSQL_DATABASE", "mydb", "address where mysql db is listening")
		dbDriver = "mysql"
	)
	envflag.Parse()
